...
def plot_error_vs_cost(df: pd.DataFrame, manifest: Manifest, profile: str, out_dir: Path) -> Path:
    """Left: how far off the prediction was. Right: how big the visible correction was when
    truth disagreed with it. Shown together deliberately -- an aggressive predictor can look
    great on the left and terrible on the right (frequent, large snaps), which is exactly the
    tradeoff this pair of panels is meant to make visible.
    """
